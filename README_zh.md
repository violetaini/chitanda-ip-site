...
- 支持裸 `curl ip.chitanda.net` 输出纯文本客户端 IP。
- 支持 IP 归属查询和地图降级策略。
...
未设置 `VITE_GEOIP_BASE` 时，本地开发使用 Chitanda GeoIP 接口，生产环境使用同源 `/api/geoip`。Chitanda 接口限制为本站使用，fork 或自部署必须自行部署 GeoAPI，并在构建前设置 `VITE_GEOIP_BASE`。

别人 fork 或自部署时，修改 `.env.production` 后再执行 `npm run build` 或 GitHub Actions 发布构建即可。仓库内的 `.env.production` 是官方站点参考配置；请把官方值替换为自己的站点名、首页大标题、域名、公开 API 基础地址、头像/favicon、GeoIP 与探针端点、公开浏览器地图 Key、可选公开 STUN、可选 CDN 探针。除非你在同源反代 `/api/geoip`，否则必须把 `VITE_GEOIP_BASE` 设置为自己的 GeoAPI。`.env.production.local` 优先级更高且被 git 忽略，只用于本地或明确不会发布到 GitHub Release 的私有服务器构建。
...
这个仓库发布的是前端。fork 或自部署必须自行运行 GeoAPI，因为默认的 Chitanda 接口限制为本站使用。可使用完全开源的 API 项目：
...
如果要提供命令行式 IP 回显，需要在边缘和反向代理里配置例外，让裸 `curl ip.chitanda.net` 到达 GeoAPI 的 `/myip?format=text`，而不是在到达源站前就被强制跳转到 HTTPS。
